...
def _make_service(
    ...
    return RepositoryResearchService(settings, provider=InMemoryProvider(files))
...
async def test_search_result_has_stable_id() -> None:
    svc = _make_service(repos=["owner/repo"], files={"file.py": "content"})
...
async def test_search_result_has_canonical_url() -> None:
    svc = _make_service(repos=["owner/repo"], files={"src/app.py": "data"})
...
async def test_search_respects_limit() -> None:
    ...
    svc = _make_service(repos=["owner/repo"], files=files, max_results=5)
...
async def test_search_enforces_max_results_cap() -> None:
    files = {f"file{i}.py": "match" for i in range(20)}
    svc = _make_service(repos=["owner/repo"], files=files, max_results=3)
...
async def test_search_denied_for_unknown_repo() -> None:
    svc = _make_service(repos=["owner/repo"], files={})
...
async def test_search_empty_results() -> None:
    svc = _make_service(repos=["owner/repo"], files={"file.py": "nothing"})
...
async def test_fetch_returns_document() -> None:
    svc = _make_service(repos=["owner/repo"], files={"README.md": "# Hello\nWorld"})
...
async def test_fetch_url_is_canonical() -> None:
    svc = _make_service(repos=["owner/repo"], files={"src/lib.py": "x = 1"})
...
async def test_fetch_truncates_large_files() -> None:
    large_content = "x" * 200
    svc = _make_service(repos=["owner/repo"], files={"big.py": large_content}, max_fetch_bytes=50)
